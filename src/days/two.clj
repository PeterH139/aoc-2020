...
(def question-1-input ["8-11 l: qllllqllklhlvtl"
                       "1-3 m: wmmmmmttm"
                       "2-4 p: pgppp"
...
(defn validate-1 [min max char pw]
  (as-> (frequencies pw) x
        (get x char)
        (or x 0)
        (<= min x max)))

(defn validate-2 [i j char pw]
  (let [x (and (<= (dec i) (count pw))
               (= char (nth pw (dec i))))
        y (and (<= (dec j) (count pw))
               (= char (nth pw (dec j))))]
    (and (or x y)
         (not (and x y)))))

(defn parse [line]
  (let [[range c pw] (str/split line #" ")
...
(defn question-1-answer [] (answer validate-1))
(defn question-2-answer [] (answer validate-2))